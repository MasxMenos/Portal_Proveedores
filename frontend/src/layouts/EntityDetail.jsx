// src/layouts/DetalleEntidadPage.jsx
...
import EntidadMaster from "../components/detail/EntityMaster";
...
export default function EntidadDetail({ tipo }) {
  ...
  const measurePositions = useMeasurePositions(masterRef, containerRect, lines, childRefs, setPositions);

  useLayoutEffect(() => { measurePositions(); }, [measurePositions]);

  useEffect(() => { window.scrollBy({ top: 1, behavior: "smooth" }); }, [lines]);
  ...
  return (
    <div className={`flex flex-col md:flex-row min-h-screen ${isDark ? "bg-black text-white" : "bg-white text-black"}`}>
      ...
      <main className="flex-1 p-6 md:ml-64 space-y-6 overflow-auto">
        <HeaderSuperior activePage={`${tipo}/${documentoId}`} title={`Estado ${tipo}`} onToggleTheme={toggleTheme} />
        <button className="flex items-center gap-2 text-gray-400 hover:underline cursor-pointer" onClick={() => navigate(-1)}>
          <ArrowLeftCircle size={20} /> Volver
        </button>

        <div ref={containerRef} className="relative w-full h-[150vh] p-4 md:p-8">
          {containerRect && positions.master && (
            <svg className="absolute inset-0 pointer-events-none z-0" width={containerRect.width} height={containerRect.height}>
              {Object.entries(positions.children).map(([id, pos]) => {
                ...
                return (
                  <polyline key={id} points={`${sx},${sy} ${midX - containerRect.x},${sy} ${midX - containerRect.x},${ey} ${ex},${ey}`} fill="none" stroke="#888" strokeWidth="2" strokeDasharray="4 2" />
                );
              })}
            </svg>
          )}

          <EntidadMaster isDark={isDark} master={master} ref={masterRef} />

          {lines.map((line, idx) => {
            if (!childRefs.current[line.id]) childRefs.current[line.id] = React.createRef();
            const itemsPerPage = 5;
            const currentPage = paginationStates[line.id] || 1;
            const start = (currentPage - 1) * itemsPerPage;
            const end = start + itemsPerPage;
            const paginatedData = line.movements.slice(start, end);
            return (
              <DraggableLine
                key={line.id}
                ...
                setPaginaActual={(page) => setPaginationStates((prev) => ({ ...prev, [line.id]: page }))}
              />
            );
          })}
        </div>
